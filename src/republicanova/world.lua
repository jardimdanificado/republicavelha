...
function findTrunkGrowPosition(collisionMap,x,y,z)--this try to find a air block in the 9 above blocks
    ...
    for i = 1, #directions do
        opt = directions[i]
        if((x+opt.x >0 and y+opt.y>0 and x+opt.x < #collisionMap and y+opt.y < #collisionMap[0])~= true) then
...
function findRootGrowPosition(collisionMap,x,y,z)--this try to find a air block in the 9 below blocks
    ...
    for i = 1, #directions do
        opt = directions[i]
        if((x+opt.x >=0 and y+opt.y>=0 and z+opt.z >=0 and x+opt.x < #collisionMap and y+opt.y < #collisionMap[0] and z+opt.z < #collisionMap[0]) ~= true)then
        elseif(collisionMap[x+opt.x][y+opt.y][z+opt.z]  <75) then
            return opt
        end
    end
...
function gravity(collisionMap,position)
    ...
    elseif(collisionMap.check({x=position.x,y=position.y,z=position.z-1})) then
        return{x=position.x,y=position.y,z=position.z-1}
    end
    return(position)
end
...
function seedFrame(world,plant)
    if(type(world.map.block[plant.position.x][plant.position.y][plant.position.z-1]) ~= nil) then
        plant.position = gravity(world.map.collision,plant.position)
        if(world.time%60==0 and Materials[world.map.block[plant.position.x][plant.position.y][plant.position.z-1]].name == 'earth') then
            ...
            if(plant.status == 'germinating') then
                if(plant.germination >= Plants[plant.specie].time.maturing.max/100000 or (util.roleta(19,1) == 1 and plant.germination>=(Plants[plant.specie].time.maturing.min/100000))) then
...
function growLeaf(plant)
    if(Util.rolocala(14,1,16) == 1) then
...
function growBranch(plant,collisionMap,time)
    if(type(plant.trunk) ~= nil and #plant.trunk>=1 and #plant.branch < Plants[plant.specie].leaf.max/1000 and util.roleta(2,1,2) == 1) then
        local customX = Util.rolocala(1,0,1)-1
        local customY = Util.rolocala(1,0,1)-1
        
        table.insert(plant.branch,1,types.branch(plant.specie,'idle',time,{x=customX,y=customY,z=0},plant.quality,plant.condition));
        local collisionPositions = (#plant.trunk>1) and {plant.position,plant.trunk[1].position,plant.branch[0].position} or {plant.position,plant.trunk[0].position,plant.branch[0].position}
...
function growTrunk(plant,collisionMap,time)
    if(type(plant.trunk) ~= nil and #plant.trunk < (Plants[plant.specie].size.max/100)) then
        local customX = Util.rolocala(1,10,1)-1;
        local customY = Util.rolocala(1,10,1)-1;
        ...
        table.insert(plant.trunk,1,types.trunk(plant.specie,'idle',time,{x=0,y=0,z=0},plant.quality,plant.condition))
...
function plantFrame(world,plant)
    if(plant.leaf < Plants[plant.specie].leaf.max and world.time % Util.LimitTo(Plants[plant.specie].time.maturing.min,0,10)==0) then
        if(Plants[plant.specie].size.max > 100) then
            if(world.time % Util.LimitTo(Plants[plant.specie].time.maturing.min,0,10)==0) then
    ...
    if(Plants[plant.specie].type.includes('tree')) then
        ...
        if(world.time % util.math.limit(Plants[plant.specie].time.maturing.min,1,100)==0 and lastTrunkPosition.x < #world.map.block[0][0]) then
...
function frame(world)
    ...
    if(#world.plant>0) then
        world.plant = util.matrix.map(world.plant,function(plant)
...
function Map(multiHorizontal,quality)--create the map
    ...
    local collision = 
    {
        ...
        check=function(position,value)--returns true if no collider in the specified position, of if the colliders in the position are below value;
            value = value or 75
